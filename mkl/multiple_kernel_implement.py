...
import numpy as np
import mkl.kernel_helper_all as helpers
...
# Geometric interpretation: maximize the margin between positive and negative samples with specific linear
#  combination of kernels

# Input parameter:
# d_init:initial kernel weight value vector d={d1,d2...dk} as a starting point for search.
# kernel_matrices: all pre-computed kernel matrices based on input sample matrix X which has been summarized
# by kernel function
...
def find_kernel_weights(d_init, kernel_matrices, C, y, verbose, gamma):
    # Initialization, starting from a point d
    # weights below this value are set to 0
    weight_precision = 1e-08
    goldensearch_precision = 1e-01
    goldensearch_precision_init = 1e-01
    max_goldensearch_precision = 1e-08
    # search stopping criteria defined in the paper
    duality_gap_threshold = 0.01
    for m in kernel_matrices:
        assert m.shape == (y.shape[0], y.shape[0])
    # how many kernels we have
    M = len(kernel_matrices)
    # initial guessed weights of each kernel, d_m=1/M, where M is the number of kernels
    d = d_init
    ...
    while not stop_state:
        ...
        if duality_gap < duality_gap_threshold:
            stop_state = True
        iteration += 1
    return d, k_helpers.get_combined_kernel(kernel_matrices, d), J, alpha, duality_gap, gamma